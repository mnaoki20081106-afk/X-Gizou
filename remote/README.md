...
リモートChromiumは `--app=https://x.com/home` で起動し、通常のタブバー・アドレスバーを出さないアプリモードを使います。
リモート画面は393×852の縦長表示に固定し、Xのレスポンシブなモバイルレイアウトが使われる構成です。
Selkiesの管理サイドバー・ロゴ・主要なデスクトップ操作ボタンも通常画面から隠します。iPhone側ではX-Gizou自身の下部タブバーとブラウザ風ヘッダーを隠し、Xを画面の主表示にします。

X-Gizouの設定へ戻る場合は、X画面右上に重ねて表示する小さな「…」メニューを使います。再接続、プロフィール、BANチェック、環境、設定へ移動できます。
日本語入力のためSelkiesのオンスクリーンキーボード呼び出し機能は残しています。
